...
function LeadsSection({ businessId, onDelete }) {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data } = await supabase.from('leads').select('id,name,email,contact,subject,created_at').eq('business_id', businessId).order('created_at', { ascending: false });
      setRows(data || []);
      setLoading(false);
    })();
  }, [businessId]);
