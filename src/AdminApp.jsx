...
function AdminDashboard({ session, onSignOut }) {
  ...
  async function uploadDocument(event) {
    ...
    try {
      ...
      const { data, error: insertError } = await supabase.from('knowledge_documents').insert({ business_id: businessId, title: file.name.replace(/\.[^.]+$/, ''), file_name: file.name, file_type: file.type, storage_path: path, extracted_text: extractedText, status: extractedText ? 'ready' : 'uploaded' }).select('id,title,file_name,file_type,storage_path,status,created_at').single();
      if (insertError) { await supabase.storage.from('business-knowledge').remove([path]); throw insertError; }
      setDocuments(current => [data, ...current]); setNotice(`✓ ${file.name} uploaded successfully.`);
    } catch (err) { setError(`File upload failed: ${err.message || 'Unknown error'}`); } finally { setUploading(false); }
  }
